...
const promptFragment = (market: Market) => `Format for Depop. Depop has NO separate title field — the listing is one description of up to 1000 characters, and its FIRST LINE is what shows in the feed, so treat line 1 as a keyword-dense pseudo-title: 3-5 plain words a buyer would type (brand, garment, ${isAmerican(market) ? "color" : "colour"} or fit) + size. Aesthetic words (y2k, grunge, cottagecore) go AFTER the first line, never in it; Depop's own sellers' training says they do not help the title. Put the value you would use as a title into BOTH the "title" field (for bower's own UI) and as the opening line of "description".
...
- { "label": "Product type", "value": "<e.g. 'T-shirts', 'Hoodies', 'Jeans', 'Jackets', '${isAmerican(market) ? "Sneakers" : "Trainers"}'>" }
- { "label": "Brand", "value": "<brand or 'Unbranded'>" }
- { "label": "Size", "value": "${isAmerican(market) ? "<US size>" : "<UK/EU size>"}" }
- { "label": "Condition", "value": "<MUST be one of: Brand new | Used – like new | Used – excellent | Used – good | Used – fair>" }
- { "label": "${isAmerican(market) ? "Color" : "Colour"}", "value": "<up to 2 ${isAmerican(market) ? "colors" : "colours"}>" }
