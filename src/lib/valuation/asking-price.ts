 ...
 */
function webSearchTool(platform: Platform): Anthropic.Messages.WebSearchTool20260209 {
  return {
    type: "web_search_20260209",
    name: "web_search",
    max_uses: 2,
    user_location: { type: "approximate", country: "GB" },
    allowed_domains: [...platformMetadata[platform].searchDomains],
  };
}

/** A URL that opens one listing on this platform, or undefined. */
export function listingUrl(url: unknown, platform: Platform): string | undefined {
  if (typeof url !== "string") return undefined;
  const trimmed = url.trim();
  return platformMetadata[platform].itemUrl.test(trimmed) ? trimmed : undefined;
}
...
- Every comparable's "url" must open THAT ONE LISTING — a page of the form ${meta.itemUrlExample}. A brand page, a search or catalogue page, or the homepage is not a listing and will be discarded, so do not list an item unless you have its own page.
...
export function coerceBand(raw: RawBand, platform: Platform): PriceBand {
  ...
  const comparables = Array.isArray(raw.comparables)
    ? raw.comparables
        ...
        .flatMap((c): Comparable[] => {
          const url = listingUrl(c.url, platform);
          if (!url) return [];
          return [
            {
...
export const askingPriceProvider: ValuationProvider = {
  async band(item: ValuationItem, platform: Platform): Promise<PriceBand> {
    ...
    try {
      ...
      const band = coerceBand(parseStructuredContent<RawBand>(response.content), platform);
      generation?.finish({
        output: band,
