// Chip vocabulary shared with the iOS app (RefinementChip in
// ios-app/bower/bower/Models.swift). Both send the same `instruction` strings
// to /api/refine; `usInstruction` is what a US seller's app sends instead.
...
export const CHIPS: { id: ChipId; label: string; instruction: string; usInstruction?: string }[] = [
  ...
  { id: "measurements", label: "+ Measurements",     instruction: 'Add a measurements line for the seller to complete, laid flat, in cm, suited to the garment (e.g. "Pit to pit __ cm, length __ cm, sleeve __ cm"; waist and inseam for trousers). Only if not already present. Never invent a number.',
    usInstruction: 'Add a measurements line for the seller to complete, laid flat, in inches, suited to the garment (e.g. "Pit to pit __ in, length __ in, sleeve __ in"; waist and inseam for pants). Only if not already present. Never invent a number.' },
...
export function instructionsFor(chipIds: Iterable<ChipId>, market?: Market): string[] {
  const set = new Set(chipIds);
  const us = market !== undefined && isAmerican(market);
  return CHIPS.filter((c) => set.has(c.id)).map((c) => (us && c.usInstruction) || c.instruction);
}
